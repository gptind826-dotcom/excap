...
public class ConnectionDescriptor implements HTTPReassembly.ReassemblyListener {
    ...
    public void processUpdate(ConnectionUpdate update) {
        ...
        if((update.update_type & ConnectionUpdate.UPDATE_PAYLOAD) != 0) {
            ...
            assert(decryption_ignored || isNotDecryptable() || ExcapApp.getInstance().isDecryptingPcap());
    ...
    public DecryptionStatus getDecryptionStatus() {
        ...
        else if(decryption_ignored || (ExcapApp.getInstance().isDecryptingPcap() && !internal_decrypt))
    ...
    public boolean isNotDecryptable()   { return !decryption_ignored && (encrypted_payload || !mitm_decrypt) && !ExcapApp.getInstance().isDecryptingPcap(); }
